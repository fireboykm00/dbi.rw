      ...
      <div className="relative z-10 max-w-7xl 2xl:max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6 2xl:gap-8 mb-16 2xl:mb-20">
          {currentItems.map((platform, index) => (
            <div
              key={index}
              className="group relative bg-white rounded-3xl p-2 border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_15px_40px_rgba(3,57,108,0.08)] transition-all duration-300 flex flex-col h-full"
            >
              {/* Logo Area with snappier transitions */}
              <div className="relative aspect-square md:aspect-auto md:h-[220px] bg-gray-50/50 rounded-2xl flex items-center justify-center p-10 overflow-hidden group-hover:bg-brand-primary/5 transition-colors duration-300">
              ...
              <div className="p-5 flex flex-col flex-1">
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="flex-1">
                    <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold bg-brand-primary/5 text-brand-primary uppercase tracking-wider mb-2">
                      {platform.sector}
                    </span>
                    <h3 className="text-xl font-bold text-gray-900 group-hover:text-brand-primary transition-colors duration-200 leading-tight">
                      {platform.name}
                    </h3>
                  </div>

                  <div className="mt-1 w-10 h-10 rounded-full bg-brand-primary text-white flex items-center justify-center scale-90 opacity-0 -translate-y-2 group-hover:scale-100 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 shadow-lg shadow-brand-primary/20">
